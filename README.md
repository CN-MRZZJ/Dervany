...
- **积分规则** — 个人/团体项目名次对应积分，支持动态增删
- **成绩策略** — 项目类型（event-type）CRUD，代号 + 显示名 + 比较策略（time/length/count/count_miss），通过 `/api/v1/event-types` 管理
- **组别选项** — 运动员/项目可选组别值配置

### 轮次成绩公示

支持按 `attempt_number` 筛选导出单轮成绩（xlsx / pdf 预览）。

## API 字段映射

| 旧字段 | 新字段 | 说明 |
|--------|--------|------|
| `age_group` | `group` | 运动员/项目/成绩/队伍的组别字段 |
| `age_group_options` | `group_options` | 规则配置中的组别选项 |
| `age_group_text` | `group_text` | 进度查询中的组别显示文本 |
